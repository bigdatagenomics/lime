...
class ComplementSuite extends LimeFunSuite {
  sparkTest("testing complement on large bed file") {
    ...
    val referenceRegionKeyedLeft = leftRdd.rdd.map(f => (ReferenceRegion.unstranded(f), f))

    val complements = DistributedComplement(referenceRegionKeyedLeft, leftRdd.optPartitionMap.get, genomeMap)
      .compute()
      .map(_._1)
      .collect()
